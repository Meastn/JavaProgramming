...
public class Diving {
    public static void main(String[] args) {
        ...
        float[] result = new float[score.length - 2];
        int j = 0;
        for (int i = 0; i < score.length; i++) {
            if (i == 0) { // if the index of array is matching the given index
                continue; // skip
            }
            if (i == score.length-1) {
                continue;
...
/*
In the sport of diving, seven judges
award a score between 0 and 10, where each
score may be a floating-point value.

The highest and lowest scores are thrown out,
 and the remaining scores are added together.
 The sum is then multiplied by the degree of difficulty
 for that dive.

The degree of difficulty ranges from 1.2 to 3.8 points.
The total is then multiplied by 0.6 to determine
the divers score.

>Note: System.out.printf("Total: %.2f",total);
is used to get rid of extra floating points.
